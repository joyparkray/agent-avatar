...
| **表情 / 动作** | 一张表里给每一项挑触发方式：单击、双击，或一个**全局快捷键**（桌宠没有焦点时也管用）。同一个触发绑多项就在它们之间随机。 |
| **别名** | 第三方模型的名字基本不能看（`F1`、`2222333`）。作者在模型里起过的名字会自动填上，不满意可以自己改。 |
...
| **声音来源** | 默认「无」，你选了才开始采集：系统音频、音频文件、Hermes 语音流 —— 按你实际的语音方案选。 |
...
**Agent**（接入 connector，以及为每个状态指定动作）、**行为**（闲置自治、表情与动作的触发方式与别名）、
**模型**（安装、隐藏、删除）。
