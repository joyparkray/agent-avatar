...
🔴 **Windows 上 `codex` 不在 PATH 里，但它存在**：ChatGPT app 把它放在
`%LOCALAPPDATA%\OpenAI\Codex\bin\<hash>\codex.exe`，目录名是构建哈希，
取最新的那个、用绝对路径调。

`localize.py` 负责把解释器绝对路径写进 hook 命令行。Codex 有 `commandWindows`
这个 Windows 专用命令覆盖字段，所以 POSIX 那条 `/usr/bin/python3` **原样保留**，
两个平台共用同一份 hooks.json。完整说明（含另外四家）见
[连接器 README](../marketplace-README.zh.md)。

然后：**完全退出并重开 ChatGPT app** → 在 **Plugins 标签页启用 Agent Avatar** →
**审阅并信任它的 hooks**。
...
`codex plugin marketplace add ./` 把构建出来的树登记成本地 marketplace（写进
`~/.agents/plugins/marketplace.json`）；`codex plugin add` 再把插件拷进
`~/.codex/plugins/cache/…`，并把那份拷贝报告为已安装的根目录。

> 🔴 **marketplace 条目里的路径必须是相对于 marketplace root 的 `./` 路径**，且必须留在
> root 内。官方：*"Codex resolves `source.path` relative to the marketplace root"*。
> **给绝对路径会被静默丢弃** —— 不报错，插件在 `codex plugin list` 里根本不出现，
> app 的 Plugins 标签页里也看不到。我们第一版手写的注册就是这么错的，2026-08-28 实测撞到。
> 用 `codex plugin marketplace add` 登记就绕开了整个问题。
>
> 排查用 `codex plugin list`（不用开 app）：
> - 完全没有这一行 → marketplace 根本没加上，或者路径写错了
> - `not installed` → 登记到了但没装，跑 `codex plugin add agent-avatar@agent-avatar`
...
卸载：`codex plugin remove agent-avatar`，再 `codex plugin marketplace remove agent-avatar`。
...
Event      PreToolUse
Matcher    *
Source     Plugin — agent-avatar@agent-avatar  ← 这一行就是标识
Command    /usr/bin/python3 …/agent-avatar/<版本>/scripts/agent-avatar-hook.py ; exit 0
Timeout    600s
...
**Codex.app 侧未实机验证**：验证是在 codex-cli 0.144.4 上做的，没装 Codex.app。
事件形状按官方文档实现，**装了 desktop 之后建议按 Claude Code 那套方法实抓一轮复核** ——
用隔离配置跑一个只记 stdin 的取样脚本，比对本页 §3 的字段。
