...
大多数桌宠都焊死在一个应用和一个角色上。Agent Avatar 是夹在两者中间的一层，**两头都能换**。

| 你的 agent | 你的形象 |
|---|---|
| **五家开箱即用** —— Claude Code、Codex、Hermes、DeepSeek Harness、WorkBuddy | **任何 Live2D Cubism 模型** —— 买的、画的、官网下的免费模型都行 |
| **app 内一键接入**，不用开终端、不用找脚本 | **想装多少装多少**，右键菜单里随时换，不重启 |
| **想接自己的 harness 也行** —— 状态约定是一份很小的公开[协议](bridge/README.md) | **怎么演你说了算** —— 每个 agent 状态配哪个动作可以自己指定，也可以交给模型的 `avatar.json` |
| 同时开着好几个 agent 时，形象跟着最近活动的那个走，也可以钉死在某一家 | 直接用模型自带的动作、表情和口型参数，**不需要为我们额外做适配** |
...
- **不知道去哪找**：首次启动的引导卡片里直接给了 [Live2D 官方免费示例模型](https://www.live2d.com/zh-CHS/learn/sample/)
  的链接，下载解压后拖进来就能用，几分钟的事。
- **已经有喜欢的模型**：直接换上，任何 Cubism 3/4/5 模型都行，不需要为我们做任何适配。
...
- **一眼看懂它在干嘛**：思考、执行、等待输入、审阅、同步、出错各有各的样子，
  外加「受阻」和「被打断」两种叠加反应。
- **让会说话的 agent 有张脸**：口型可由系统音频、本地音频文件或 Hermes 语音流驱动，
  不绑定某一种语音方案。
- **在场，但不碍事**：透明悬浮、窗口置顶，人物轮廓之外的点击自动穿透；
...
connector 只告诉形象 **agent 正在做什么**，形象则根据你的模型和设置决定**怎么演**。
两边通过轻量的 [Bridge Protocol](bridge/README.md) 通信，因此 agent、Live2D 模型和动作映射可以各自替换。
...
| **模型画廊** | 一屏对比所有已装模型：画布尺寸、有多少动作和表情、`avatar.json` 的映射有没有问题。坏模型在上桌之前就能发现。 |
| **聚焦模式** | 只显示胸像而不是全身，占地更小；裁多少由你调。 |
| **表情 / 动作** | 菜单里可以直接播；单击形象随机换表情，双击随机播动作 —— 而且**哪些进随机池由你勾**。 |
| **闲置自治** | 静置 N 秒后自己看四周、播动作，用的是**另一份名单**：打哈欠适合自己待着时做，当作「你点我一下」的回应就很怪。填 0 即关闭。 |
| **画质与帧率** | 三档画质 + 30/60 帧。降画质通常比降帧率更省 GPU，菜单里就是这么写的。 |
