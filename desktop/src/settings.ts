...
const TEXT: Record<Language, Record<string, string>> = {
  "zh-CN": {
    "tab.general": "通用", "tab.video": "视频", "tab.agent": "Agent", "tab.behavior": "行为", "tab.about": "关于", "about.author": "晓", "about.update": "更新", "about.xhs": "小红书号", "general.updateNow": "手动查一次当前版本是不是最新的", "about.rights": "保留所有权利。", "about.repo": "源码仓库", "about.thirdParty": "第三方组件", "about.thirdPartyHint": "本应用用到下列组件，版权归各自作者所有。", "about.live2d": "Live2D 专有软件许可", "tab.models": "模型",
  ...
  en: {
    "tab.general": "General", "tab.video": "Video", "tab.agent": "Agent", "tab.behavior": "Behavior", "tab.about": "About", "about.author": "Xiao", "about.update": "Updates", "about.xhs": "Xiaohongshu", "general.updateNow": "Check once whether this is the latest version", "about.rights": "All rights reserved.", "about.repo": "Source repository", "about.thirdParty": "Third-party components", "about.thirdPartyHint": "This app uses the components below; each remains the property of its author.", "about.live2d": "Live2D Proprietary Software License", "tab.models": "Models",
...
async function boot(): Promise<void> {
  ...
  guard("about-links", () => {
    // 🔴 **限定在关于面板里，而且属性名不能叫 data-link。** 原来这里是全文档扫
    // `[data-link]` —— 而连接器列表用同名属性存的是**连接状态**（`row.dataset.link =
    // "missing"`，见 connectors.ts）。于是点一下连接器那一行，就会去打开一个叫
    // "missing" 的地址，界面上冒出「这个地址不在允许打开的名单里：missing」。
    // 改名 + 限定范围，两道都做：撞车一次就够了。
    const panel = $<HTMLElement>('[data-panel="about"]');
    panel.querySelectorAll<HTMLElement>("[data-open-url]").forEach(node => {
