...
| `hermes plugins install` | 注册方式 —— **不碰用户的 config.yaml**。Hermes 只从 git 装，所以要先把构建出来的树提交进一个临时仓库，见[连接器 README](../marketplace-README.zh.md) |
| — | Hermes 是五家里**唯一不需要本地化解释器**的一家：插件是 in-process 的 Python 包，跑在 Hermes 自己的解释器里、不 spawn 进程，所以「`python3` 在 Windows 上是应用商店存根」那颗雷碰不到它 —— 也因此它是 Windows 接线的**基线** |
...
| `../../desktop/src-tauri/src/hermes.rs` | `read_semantic_state` / `discover_audio_endpoint` 两个 Tauri 命令 |
| `../../docs/CONNECTORS.md` | 用户如何安装 |
...
1. 删掉本目录（保留 `../../bridge/`）。
2. 删 `desktop/src-tauri/src/hermes.rs`，并从 `lib.rs` 去掉 `mod hermes;` 与 `generate_handler!` 里那两个
   `hermes::` 命令。
